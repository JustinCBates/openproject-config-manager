...
class InteractiveCollector:
    ...
    def _get_default_config_data(self, discovered_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        default_domain = (
            system_recommendations.get("network", {}).get("suggested_domain")
            or docker_recommendations.get("networking", {}).get("suggested_domain")
            or "openproject.local"
        )
    ...
    def _collect_core_settings(
        self, config_data: Dict[str, Any], discovered_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if not config_data.get("secret_key_base"):
            ...
            if self.ui.confirm(
                "Regenerate secret key? (Warning: This will invalidate existing sessions)"
            ):
                config_data["secret_key_base"] = self._generate_secret_key()
    ...
    def _collect_database_config(
        self, db_config: Dict[str, Any], discovered_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if db_containers:
            ...
            for container in db_containers:
                self.ui.show_info(
                    f"  - {container['name']} ({container.get('database_type', 'unknown')})"
                )

        # Database adapter
        adapter_choices = ["postgresql", "mysql"]
        suggested_adapter = db_recommendations.get(
            "suggested_adapter", db_config.get("adapter", "postgresql")
        )

        db_config["adapter"] = self.ui.select(
            "Select database adapter", choices=adapter_choices, default=suggested_adapter
        )

        # Database host
        suggested_host = db_recommendations.get("suggested_host", db_config.get("host", "db"))
        db_config["host"] = self.ui.prompt("Database host", default=suggested_host)
        ...
        db_config["name"] = self.ui.prompt(
            "Database name", default=db_config.get("name", "openproject")
        )

        # Database username
        db_config["username"] = self.ui.prompt(
            "Database username", default=db_config.get("username", "openproject")
        )
        ...
        db_config["encoding"] = self.ui.prompt(
            "Database encoding", default=db_config.get("encoding", "utf8")
        )

        return db_config

    def _collect_proxy_config(
        self, proxy_config: Dict[str, Any], discovered_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        # Primary domain
        suggested_domain = network_recommendations.get(
            "suggested_domain", proxy_config.get("domain", "openproject.local")
        )
        proxy_config["domain"] = self.ui.prompt("Primary domain", default=suggested_domain)
        ...
        proxy_config["ssl_enabled"] = self.ui.confirm(
            "Enable SSL/TLS?", default=proxy_config.get("ssl_enabled", True)
        )
        ...
        if proxy_config["ssl_enabled"]:
            ...
            if proxy_config["lets_encrypt"]:
                ...
                proxy_config["ssl_cert_path"] = self.ui.prompt(
                    "Path to SSL certificate file", default=proxy_config.get("ssl_cert_path", "")
                )
                proxy_config["ssl_key_path"] = self.ui.prompt(
                    "Path to SSL private key file", default=proxy_config.get("ssl_key_path", "")
                )

        # Reverse proxy
        proxy_config["reverse_proxy_enabled"] = self.ui.confirm(
            "Enable reverse proxy?", default=proxy_config.get("reverse_proxy_enabled", True)
        )

        return proxy_config

    def _collect_url_config(
        self, config_data: Dict[str, Any], discovered_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if uri_namespace_enabled:
            ...
            self.ui.show_success(
                f"OpenProject will be accessible at: https://{proxy_domain}{uri_namespace}"
            )
    ...
    def _collect_storage_config(
        self, storage_config: Dict[str, Any], discovered_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        storage_config["data_volume"] = self.ui.prompt(
            "Data volume name", default=storage_config.get("data_volume", "openproject_data")
        )

        # Logs volume
        storage_config["logs_volume"] = self.ui.prompt(
            "Logs volume name", default=storage_config.get("logs_volume", "openproject_logs")
        )

        # Backup configuration
        storage_config["backup_enabled"] = self.ui.confirm(
            "Enable automated backups?", default=storage_config.get("backup_enabled", True)
        )
        ...
        if storage_config["backup_enabled"]:
            ...
            # Backup location
            suggested_location = storage_recommendations.get(
                "backup_location", storage_config.get("backup_location", "./backups")
            )
            storage_config["backup_location"] = self.ui.prompt(
                "Backup location", default=suggested_location
            )

        return storage_config

    def _collect_email_config(
        self, config_data: Dict[str, Any], discovered_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if config_data["email_delivery_method"] == "smtp":
            ...
            config_data["smtp_address"] = self.ui.prompt(
                "SMTP server address", default=config_data.get("smtp_address", "")
            )

            config_data["smtp_port"] = self.ui.prompt_int(
                "SMTP port", default=config_data.get("smtp_port", 587), min_value=1, max_value=65535
            )

            config_data["smtp_domain"] = self.ui.prompt(
                "SMTP domain", default=config_data.get("smtp_domain", ""), allow_empty=True
            )

            config_data["smtp_user_name"] = self.ui.prompt(
                "SMTP username", default=config_data.get("smtp_user_name", ""), allow_empty=True
            )
            ...
            config_data["smtp_enable_starttls_auto"] = self.ui.confirm(
                "Enable STARTTLS?", default=config_data.get("smtp_enable_starttls_auto", True)
            )

        return config_data

    def _collect_performance_config(
        self, config_data: Dict[str, Any], discovered_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        # Web concurrency
        suggested_concurrency = perf_recommendations.get(
            "web_concurrency", config_data.get("web_concurrency", 2)
        )
        config_data["web_concurrency"] = self.ui.prompt_int(
            "Web worker processes", default=suggested_concurrency, min_value=1, max_value=16
        )
        ...
        if config_data.get("rails_cache_store") == "memcache":
            config_data["memcached_server"] = self.ui.prompt(
                "Memcached server", default=config_data.get("memcached_server", "cache:11211")
            )
        elif config_data.get("rails_cache_store") == "redis":
            config_data["redis_url"] = self.ui.prompt(
                "Redis URL", default=config_data.get("redis_url", "redis://redis:6379/0")
            )
    ...
    def _collect_security_config(
        ...
        config_data["force_ssl"] = self.ui.confirm(
            "Force SSL/HTTPS?", default=config_data.get("force_ssl", True)
        )

        # Session cookie security
        config_data["session_cookie_secure"] = self.ui.confirm(
            "Secure session cookies?", default=config_data.get("session_cookie_secure", True)
        )

        return config_data

    def _collect_feature_config(
        self, config_data: Dict[str, Any], discovered_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        config_data["log_level"] = self.ui.select(
            "Log level", choices=log_levels, default=config_data.get("log_level", "info")
        )

        # Rails logging
        config_data["rails_log_to_stdout"] = self.ui.confirm(
            "Log to stdout?", default=config_data.get("rails_log_to_stdout", True)
        )
    ...
    def _show_configuration_summary(self, configuration: Configuration):
        ...
        summary_table = Table(
            title="Configuration Overview", show_header=True, header_style="bold magenta"
        )
        ...
        if configuration.proxy.additional_domains:
            proxy_settings.append(
                f"Additional Domains: {len(configuration.proxy.additional_domains)}"
            )
        ...
        if configuration.storage.backup_enabled:
            storage_settings.append(
                f"Retention: {configuration.storage.backup_retention_days} days"
            )
