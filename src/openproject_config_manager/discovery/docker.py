...
class DockerDiscovery:
    ...
    def _find_openproject_containers(
        ...
        for container in containers:
            ...
            is_openproject = (
                "openproject" in container["name"].lower()
                or "openproject" in container["image"].lower()
                or any("openproject" in label.lower() for label in container["labels"].values())
            )
    ...
    def _find_database_containers(self, containers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for container in containers:
            is_database = any(
                keyword in container["image"].lower() or keyword in container["name"].lower()
