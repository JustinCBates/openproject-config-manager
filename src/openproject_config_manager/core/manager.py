...
class ConfigurationManager:
    ...
    def run_tui_mapping_phase(self, enhanced_defaults_path: Optional[str] = None) -> str:
        ...
        self.ui.show_phase_header(
            "TUI Mapping Phase", "Transforming defaults for TUI consumption..."
        )
        ...
        try:
            ...
            if not enhanced_defaults_path:
                enhanced_defaults_path = (
                    self.project_root / "outputs" / "discovery" / "discovery_output.yml"
                )
    ...
    def run_validation_phase(self) -> bool:
        ...
        try:
            ...
            validation_result = self.validator.validate_configuration(
                self.configuration, discovered_data=self.discovered_data
            )
    ...
    def run_export_phase(self, output_path: Optional[str] = None) -> str:
        ...
        if not self.configuration:
            raise ValueError(
                "No configuration to export. Run collection and validation phases first."
            )
        ...
        try:
            ...
            exported_path = self.exporter.write_configuration(
                self.configuration, output_path=output_path
            )
    ...
    def run_full_process(self, output_path: Optional[str] = None) -> str:
        ...
        try:
            ...
            self.ui.show_success(
                f"Configuration process completed in {duration.total_seconds():.1f}s"
            )
    ...
    def _generate_enhanced_defaults(self) -> Dict[str, Any]:
        ...
        return {"metadata": metadata, "defaults": defaults}
    ...
    def _extract_services_metadata(
        self, docker_data: Dict[str, Any], env_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _generate_domain_default(
        self, network_data: Dict[str, Any], system_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _write_tui_defaults_file(self, enhanced_defaults: Dict[str, Any]) -> str:
        ...
        header_comments = output_format.get(
            "header_comments",
            [
                "# Configuration defaults for TUI layout",
                "# Generated from discovery output via defaults mapping",
