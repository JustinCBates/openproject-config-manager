...
class ConfigurationValidator:
    ...
    def validate_configuration(self, 
                             ...
                             discovered_data: Optional[Dict[str, Any]] = None) -> ValidationResult:
        """
    ...
    def _validate_database_config(self, db_config, result: ValidationResult, 
        ...
        if not db_config.host:
            ...
            if not self._is_container_or_localhost(db_config.host):
                if not self._test_host_connectivity(db_config.host, db_config.port):
                    result.add_warning(f"Cannot reach database host {db_config.host}:{db_config.port}")
        ...
        if db_config.adapter == 'postgresql' and db_config.port != 5432:
            result.add_recommendation(f"Non-standard PostgreSQL port {db_config.port} (standard is 5432)")
        elif db_config.adapter == 'mysql' and db_config.port != 3306:
            result.add_recommendation(f"Non-standard MySQL port {db_config.port} (standard is 3306)")
        
        # Database name validation
        if not db_config.name:
            result.add_error("Database name is required")
        elif not re.match(r'^[a-zA-Z][a-zA-Z0-9_]*$', db_config.name):
            result.add_warning("Database name should start with a letter and contain only alphanumeric characters and underscores")
        ...
        if db_containers:
            ...
            if matching_containers:
                ...
                if db_config.adapter == 'postgresql' and 'postgres' not in container_type:
                    result.add_warning(f"Container {db_config.host} appears to be {container_type}, but PostgreSQL adapter is configured")
                elif db_config.adapter == 'mysql' and 'mysql' not in container_type:
                    result.add_warning(f"Container {db_config.host} appears to be {container_type}, but MySQL adapter is configured")
    ...
    def _validate_proxy_config(self, proxy_config, result: ValidationResult):
        ...
        if not proxy_config.domain:
            ...
            if proxy_config.domain in ['localhost', '127.0.0.1'] or proxy_config.domain.endswith('.local'):
        ...
        if proxy_config.ssl_enabled:
            if proxy_config.lets_encrypt:
                ...
                if not proxy_config.lets_encrypt_email:
                    result.add_error("Let's Encrypt email is required when Let's Encrypt is enabled")
                elif not self._is_valid_email(proxy_config.lets_encrypt_email):
                    result.add_error(f"Invalid Let's Encrypt email format: {proxy_config.lets_encrypt_email}")
                
                # Check domain compatibility with Let's Encrypt
                if proxy_config.domain.endswith('.local') or proxy_config.domain in ['localhost', '127.0.0.1']:
                    result.add_error("Let's Encrypt cannot issue certificates for localhost/local domains")
            else:
                # Custom SSL certificate validation
                if proxy_config.ssl_cert_path:
                    if not Path(proxy_config.ssl_cert_path).exists():
                        result.add_warning(f"SSL certificate file not found: {proxy_config.ssl_cert_path}")
                else:
                ...
                if proxy_config.ssl_key_path:
                    if not Path(proxy_config.ssl_key_path).exists():
                        result.add_warning(f"SSL private key file not found: {proxy_config.ssl_key_path}")
                else:
    ...
    def _validate_storage_config(self, storage_config, result: ValidationResult):
        ...
        if not storage_config.data_volume:
            ...
            result.add_warning("Data volume name should start with a letter and contain only alphanumeric characters, hyphens, and underscores")
        ...
        if not storage_config.logs_volume:
            ...
            result.add_warning("Logs volume name should start with a letter and contain only alphanumeric characters, hyphens, and underscores")
        ...
        if storage_config.backup_enabled:
            ...
            if not self._is_valid_cron_schedule(storage_config.backup_schedule):
                result.add_error(f"Invalid backup schedule format: {storage_config.backup_schedule}")
            ...
            if not storage_config.backup_location:
                ...
                if backup_path.is_absolute():
                    ...
                    if not backup_path.parent.exists():
                        result.add_warning(f"Backup location parent directory does not exist: {backup_path.parent}")
                else:
    ...
    def _validate_email_config(self, configuration: Configuration, result: ValidationResult):
        ...
        if configuration.email_delivery_method not in ['smtp', 'sendmail', 'letter_opener']:
            result.add_error(f"Invalid email delivery method: {configuration.email_delivery_method}")
        ...
        if configuration.email_delivery_method == 'smtp':
            ...
            if not configuration.smtp_address:
                ...
                if not self._test_host_connectivity(configuration.smtp_address, configuration.smtp_port):
                    result.add_warning(f"Cannot reach SMTP server {configuration.smtp_address}:{configuration.smtp_port}")
            ...
            if configuration.smtp_port not in standard_ports:
                result.add_recommendation(f"Non-standard SMTP port {configuration.smtp_port} (standard ports: {standard_ports})")
            ...
            result.add_warning("Letter opener is for development only - not suitable for production")
    
    def _validate_performance_config(self, configuration: Configuration, 
                                   result: ValidationResult, discovered_data: Dict[str, Any]):
        ...
        if 'cpu_count' in hardware_info:
            ...
            if configuration.web_concurrency > cpu_count * 2:
                result.add_warning(f"Web concurrency ({configuration.web_concurrency}) is high for {cpu_count} CPU cores")
        ...
        if 'memory' in hardware_info and 'total_gb' in hardware_info['memory']:
            ...
            if estimated_total > memory_gb * 0.8:  # 80% of available memory
                result.add_warning(f"High memory usage expected: {estimated_total:.1f}GB for {configuration.web_concurrency} workers")
        ...
        if configuration.web_max_requests < 100:
            result.add_warning("Very low max requests per worker may cause frequent worker restarts")
    ...
    def _validate_configuration_consistency(self, configuration: Configuration, result: ValidationResult):
        ...
        if configuration.proxy.ssl_enabled and not configuration.session_cookie_secure:
            result.add_recommendation("Consider enabling secure session cookies when SSL is configured")
        
        # Environment consistency
        if configuration.rails_env == 'development':
            if configuration.proxy.ssl_enabled and configuration.proxy.lets_encrypt:
                result.add_warning("Let's Encrypt with development environment may not work as expected")
        ...
        if configuration.database.adapter == 'postgresql' and configuration.database.port == 3306:
            result.add_warning("PostgreSQL adapter with MySQL default port - possible misconfiguration")
        elif configuration.database.adapter == 'mysql' and configuration.database.port == 5432:
            result.add_warning("MySQL adapter with PostgreSQL default port - possible misconfiguration")
    
    def _validate_environment_compatibility(self, configuration: Configuration, 
                                          result: ValidationResult, discovered_data: Dict[str, Any]):
        ...
        if 'memory' in hardware_info and 'total_gb' in hardware_info['memory']:
            memory_gb = hardware_info['memory']['total_gb']
            if memory_gb < 2:
                result.add_warning(f"Low system memory ({memory_gb:.1f}GB) - consider upgrading for production")
            elif memory_gb < 4:
                result.add_recommendation(f"Moderate system memory ({memory_gb:.1f}GB) - 4GB+ recommended for production")
        
        # Disk space check
        if 'disk' in hardware_info and 'free_gb' in hardware_info['disk']:
            free_gb = hardware_info['disk']['free_gb']
            if free_gb < 10:
                result.add_warning(f"Low disk space ({free_gb:.1f}GB free) - ensure adequate space for data and backups")
        ...
        if configuration.proxy.lets_encrypt:
            ...
            if not self._test_host_connectivity('acme-v02.api.letsencrypt.org', 443):
                result.add_warning("Cannot reach Let's Encrypt servers - check internet connectivity")
