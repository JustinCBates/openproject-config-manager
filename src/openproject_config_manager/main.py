...
@cli.command()
@click.option(
    "--output", "-o", type=click.Path(), help="Output file path (default: interactive_config.cfg)"
)
@click.pass_context
def configure(ctx, output):
    ...
    try:
        manager = ConfigurationManager(project_root=project_root, verbose=verbose)
...
@click.option(
    "--output", "-o", type=click.Path(), help="Output file path (default: update existing file)"
)
@click.pass_context
def update(ctx, config_file, output):
    ...
    try:
        manager = ConfigurationManager(
            project_root=project_root, config_file=config_file, verbose=verbose
        )
...
def collect(ctx, output_dir, flow, mock_file):
    ...
    try:
        ...
        collector = OpenProjectConfigCollector(
            flows_dir=None, output_dir=output_dir  # Use collector directory
        )
...
def validate(ctx, config_file):
    ...
    try:
        ...
        validation_result = manager.validator.validate_configuration(configuration, discovered_data)
...
def discover(ctx):
    ...
    try:
        ...
        if env_data:
            ...
            ui.show_info(
                f"Found {env_data.get('relevant_count', 0)} relevant environment variables"
            )
        ...
        if docker_data:
            ...
            if docker_data.get("docker_available"):
                ...
                if openproject_containers:
                    ui.show_warning(
                        f"Found {len(openproject_containers)} existing OpenProject containers"
                    )
