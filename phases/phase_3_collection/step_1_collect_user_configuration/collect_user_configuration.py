...
# Always use package import, fallback to None if unavailable
try:
...
class TUIConfigCollector:
    ...
    def __init__(
        ...
        if TUI_ENGINE_AVAILABLE:
            ...
            self.renderer = None
            logger.warning(
                "TUI Form Engine not available - interactive collection disabled"
            )
    ...
    def _expand_sublayouts(
        self, flow_data: Dict[str, Any], flow_path: Path
    ) -> Dict[str, Any]:
        """
        ...
        if unified_defaults:
            virtual_layout["defaults_file"] = str(
                (self.output_dir / "unified_defaults.yml").absolute()
            )
    ...
    def collect_configuration(
        ...
        try:
            ...
            flow_response = self.renderer.render_flow(
                flow_path=str(expanded_flow_path), mock_responses=None, quiet=False
            )
    ...
    def _generate_collected_config(
        self, responses: Dict[str, Any], flow_name: str
    ) -> Dict[str, Any]:
        """
    ...
    def test_flow(
        ...
        try:
            result = self.collect_configuration(
                flow_name=flow_name, mock_responses=mock_responses
            )
...
def execute_collect_user_configuration(
...
def main():
    ...
    parser.add_argument("--tui-defaults", help="Path to Phase 2 TUI defaults file")
    parser.add_argument(
        "--output-dir",
        help="Output directory (default: use PathResolver to determine phase output dir)",
    )
    parser.add_argument("--layouts-dir", help="Layouts directory (default: ./layouts)")
    parser.add_argument(
    ...
    if args.output_dir:
        ...
        try:
            path_resolver = PathResolver.from_execution_context(__file__)
            output_dir = path_resolver.resolve_phase_output_dir(
