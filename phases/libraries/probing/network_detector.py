...
class NetworkDiscovery:
    ...
    def _generate_network_recommendations(self, network_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'web' in recommended_ports:
            recommendations['ports']['web'] = {
                ...
                'alternatives': [p for p in self.OPENPROJECT_PORTS['web'] 
                              if p != recommended_ports['web']]
            }
        else:
    ...
    def _find_available_ports(self, start: int = 8000, end: int = 9000, count: int = 10) -> List[int]:
        ...
        # Check for iptables
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    firewall['active'] = len([l for l in lines if l.strip() and not l.startswith('Chain') and not l.startswith('target')]) > 0
            except Exception:
                pass
