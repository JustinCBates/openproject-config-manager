...
class DependencyValidator:
    ...
    def validate(self, configuration: Dict[str, Any]) -> DependencyValidationResult:
        ...
        result = DependencyValidationResult(
            warnings=[], recommendations=[], passed=True
        )
        ...
        logger.info(
            f"Dependency validation completed: {'PASSED' if result.passed else 'FAILED'}"
        )
        logger.info(
            f"Warnings: {len(result.warnings)}, Recommendations: {len(result.recommendations)}"
        )

        return result

    def _validate_database_dependencies(
        self, user_responses: Dict[str, Any], result: DependencyValidationResult
    ):
        ...
        if database.get("setup") == "container":
            result.add_recommendation(
                "Container database will be created - ensure Docker has sufficient resources"
            )

        # PostgreSQL vs MySQL
        if database.get("type") == "postgresql":
            result.add_recommendation(
                "PostgreSQL selected - ensure version 12+ for best compatibility"
            )
        elif database.get("type") == "mysql":
            result.add_warning(
                "MySQL is supported but PostgreSQL is recommended for OpenProject"
            )

    def _validate_email_dependencies(
        self, user_responses: Dict[str, Any], result: DependencyValidationResult
    ):
        ...
        if not email:
            result.add_warning(
                "Email configuration not provided - email notifications will not work"
            )
            return
        ...
        if delivery_method == "smtp":
            ...
            if not smtp.get("host"):
                result.add_warning(
                    "SMTP host not configured - email notifications will fail"
                )
            ...
            if port == 25:
                result.add_warning(
                    "Port 25 is commonly blocked - consider using port 587 (TLS) or 465 (SSL)"
                )
            ...
            if not encryption or encryption == "none":
                result.add_warning(
                    "SMTP encryption not enabled - consider enabling TLS/SSL for security"
                )

        elif delivery_method == "sendmail":
            result.add_recommendation(
                "Using sendmail - ensure sendmail is installed on the host system"
            )

    def _validate_storage_dependencies(
        self, user_responses: Dict[str, Any], result: DependencyValidationResult
    ):
        ...
        if backup and backup.get("enabled"):
            ...
            if retention_days > 90:
                result.add_warning(
                    f"Backup retention of {retention_days} days may consume significant storage"
                )

            result.add_recommendation(
                "Regular backups enabled - ensure backup location has sufficient space"
            )

    def _validate_network_dependencies(
        self, user_responses: Dict[str, Any], result: DependencyValidationResult
    ):
        ...
        if domain and not (
            domain in ["localhost", "127.0.0.1"] or domain.endswith(".local")
        ):
            result.add_recommendation(
                "Public domain detected - consider enabling SSL/Let's Encrypt"
            )
        ...
        if network.get("port"):
            ...
            if port != 80 and port != 443:
                result.add_warning(
                    f"Non-standard port {port} configured - ensure firewall allows this port"
                )
    ...
    def _validate_configuration_consistency(
        ...
        if project.get("environment") == "production":
            ...
            if domain in ["localhost", "127.0.0.1"] or domain.endswith(".local"):
                result.add_warning(
                    "Production environment with localhost/local domain is not recommended"
                )

            # Check for development-only settings in production
            result.add_recommendation(
                "Production environment - ensure all security settings are properly configured"
            )
        ...
        if resources:
            ...
            if worker_processes > 8:
                result.add_warning(
                    f"High worker process count ({worker_processes}) - ensure system has sufficient CPU cores"
                )
            elif worker_processes == 1:
                result.add_recommendation(
                    "Single worker process - consider increasing for better performance"
                )


def execute_dependency_validation(
    ...
    logger.info(
        f"Dependency validation completed: {'PASSED' if validation_result.passed else 'FAILED'}"
    )
    logger.info(
        f"Warnings: {len(validation_result.warnings)}, Recommendations: {len(validation_result.recommendations)}"
    )
...
class DependencyValidationStep:
    """Wrapper class for dependency_validation step."""
    ...
    def execute(self, context: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute dependency_validation step.
        ...
        result_data = execute_dependency_validation(context, self.phase_dir)
