...
class EnvironmentValidator:
    ...
    def validate(self, configuration: Dict[str, Any]) -> EnvironmentValidationResult:
        ...
        logger.info(
            f"Environment validation completed: {'PASSED' if result.passed else 'FAILED'}"
        )
        logger.info(
    ...
    def _validate_port_availability(
        self, user_responses: Dict[str, Any], result: EnvironmentValidationResult
    ):
        ...
        if unavailable_ports:
            result.add_warning(
                f"Ports already in use: {', '.join(map(str, unavailable_ports))} - ensure these can be released"
            )
    ...
    def _validate_disk_space(
        ...
        try:
            ...
            if free_gb < 10:
                result.add_warning(
                    f"Low disk space: {free_gb:.1f}GB free - OpenProject requires at least 10GB"
                )
            elif free_gb < 20:
                result.add_recommendation(
                    f"Disk space: {free_gb:.1f}GB free - consider allocating more space for production"
                )
            else:
                result.add_recommendation(
                    f"Disk space: {free_gb:.1f}GB free - sufficient for deployment"
                )

        except Exception as e:
            logger.debug(f"Could not check disk space: {e}")
            result.add_recommendation(
                "Could not verify disk space - ensure sufficient storage is available"
            )
    ...
    def _validate_network_connectivity(
        ...
        try:
            ...
            result.add_warning(
                f"Cannot resolve domain {domain} - ensure DNS is configured"
            )
        except Exception as e:
            logger.debug(f"Network check failed: {e}")

    def _validate_external_services(
        self, user_responses: Dict[str, Any], result: EnvironmentValidationResult
    ):
        ...
        if database.get("setup") == "external" and database.get("host"):
            ...
            if not self._test_host_connectivity(host, port):
                result.add_warning(
                    f"Cannot reach external database at {host}:{port} - ensure it's accessible"
                )
        ...
        if email.get("delivery_method") == "smtp":
            ...
            if smtp.get("host"):
                ...
                if not self._test_host_connectivity(host, port):
                    result.add_warning(
                        f"Cannot reach SMTP server at {host}:{port} - email notifications may fail"
                    )
...
def execute_environment_validation(
    ...
    logger.info(
        f"Environment validation completed: {'PASSED' if validation_result.passed else 'FAILED'}"
    )
    logger.info(
