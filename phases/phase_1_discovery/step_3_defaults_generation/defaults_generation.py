...
class DefaultsGenerator:
    ...
    def generate_enhanced_defaults(
        self,
        ...
        metadata = {
            ...
            "services_info": self._extract_services_metadata(
                docker_data, environment_data
            ),
...
def execute_defaults_generation(
    ...
    logger.info(
        f"Step Intelligent Defaults Generation completed: {defaults_count} defaults generated"
    )
    return result
...
def main():
    ...
    try:
        ...
        if "defaults_summary" in result:
            ...
            print(
                f"  • Total defaults generated: {summary.get('total_defaults_generated', 0)}"
            )
            print(
                f"  • Enhanced defaults file: {result.get('enhanced_defaults_file', 'N/A')}"
            )
