...
class PhasesOrchestrator:
    ...
    def _execute_hardcoded(
        ...
        try:
            ...
            logger.info(
                f"✓ Phase 1 complete: {phase1_result.get('discovery_summary', {})}"
            )
            ...
            logger.info(
                f"✓ Phase 2 complete: {phase2_result.get('transformation_summary', {})}"
            )
            ...
            logger.info(
                f"✓ Phase 5 complete: {phase5_result.get('export_summary', {})}"
            )
    ...
    def execute_phase(
        self, phase_number: int, context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def execute_phases_range(
        self, start_phase: int, end_phase: int, context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
...
def create_orchestrator(
    project_root: Path, ui=None, mode: str = "hardcoded"
) -> PhasesOrchestrator:
    """
...
Note: You can also use run_phases.py from the project root for more options.
    ...
    parser.add_argument(
        "--all", action="store_true", help="Execute all 5 phases in sequence"
    )
