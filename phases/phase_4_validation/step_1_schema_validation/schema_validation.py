...
class SchemaValidator:
    ...
    def validate(self, configuration: Dict[str, Any]) -> SchemaValidationResult:
        ...
        logger.info(f"Schema validation completed: {'PASSED' if result.passed else 'FAILED'}")
        logger.info(f"Errors: {len(result.errors)}, Warnings: {len(result.warnings)}")
        
        return result
    
    def _validate_required_sections(self, user_responses: Dict[str, Any], result: SchemaValidationResult):
    ...
    def _validate_project_section(self, project: Dict[str, Any], result: SchemaValidationResult):
        ...
        if not project.get('name'):
            ...
            if not re.match(r'^[a-z][a-z0-9-]*$', name):
                result.add_error("Project name must start with lowercase letter and contain only lowercase letters, numbers, and hyphens")
            if len(name) < 3:
        ...
        if not project.get('environment'):
            ...
            result.add_error(f"Invalid environment: {project['environment']}. Must be 'development' or 'production'")
        
        if project.get('environment') == 'development':
            result.add_warning("Development environment is not recommended for production use")
    
    def _validate_admin_section(self, admin: Dict[str, Any], result: SchemaValidationResult):
        ...
        if not admin.get('password'):
            ...
            result.add_warning("Admin password should be at least 8 characters for security")
    
    def _validate_database_section(self, database: Dict[str, Any], result: SchemaValidationResult):
        """Validate database configuration."""
        # Database type validation
        if not database.get('type'):
            result.add_error("Database type is required")
        elif database['type'] not in ['postgresql', 'mysql']:
            result.add_error(f"Unsupported database type: {database['type']}. Must be 'postgresql' or 'mysql'")
        ...
        if not database.get('setup'):
            ...
            result.add_error(f"Invalid database setup: {database['setup']}. Must be 'container' or 'external'")
    ...
    def _validate_network_section(self, network: Dict[str, Any], result: SchemaValidationResult):
...
def execute_schema_validation(context: Dict[str, Any], phase_dir: Path) -> Dict[str, Any]:
    """
    Schema Validation
    Status: IMPLEMENTED
